...
```bash
source install/setup.bash
ros2 launch agri_robot_autonomous_pkg agri_robot_rgbd_scan_demo.launch.py localization:=false ground_truth:=true #ground_truth for removing the spawn shift from the ground truth odom
```
### Running the sensor fusion node for the IMU+Laser scanner+Three RGB-D camera
```bash
source install/setup.bash
...
| /gt_odom                  | nav_msgs/msg/Odometry         | Ground truth odometry (sim) with spawn shift     |
| /gt_pose_no_drift         | nav_msgs/msg/Odometry         | Ground truth odometry (sim) with removed spawn shift     |
